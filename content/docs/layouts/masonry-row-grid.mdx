...
import { Example1 } from "./masonry-row-grid-example";
...
```
```bash tab="bun"
bunx --bun shadcn@latest add @nexul/masonry-row
```
</Tab>
<Tab>
### Manual Installation
...
```ts title="lib/merge-refs.ts" lineNumbers
...
```tsx title="components/ui/masonry/masonry-row.tsx" lineNumbers
"use client";
...
```ts title="components/ui/masonry/calculate-masonry-row.ts" lineNumbers
